...
if [ ! -f .env ]; then
    ...
    # Scoped to POST /api/internal/alert. It is what the WAF authenticates
    # with, so that container never needs the admin password (SECURE-AUTH-02).
...
INTERNAL_ALERT_TOKEN=${ALERT_TOKEN}
...
EOF
    ...
    # Top up variables added after this .env was written. Without this an
    # upgrade keeps a WAF that has no way to authenticate its alerts: the
    # shipped compose no longer hands it the admin credential, and the token
    # that replaced it would not exist. Existing values are never touched.
    if ! grep -q '^INTERNAL_ALERT_TOKEN=' .env; then
        printf '\n# Added by install.sh — service credential for POST /api/internal/alert\nINTERNAL_ALERT_TOKEN=%s\n' \
            "$(openssl rand -hex 32)" >> .env
        ok "Added INTERNAL_ALERT_TOKEN to .env (WAF alert credential)"
    fi
fi
