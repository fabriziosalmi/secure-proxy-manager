...
func TestSecurityHandlers_Score_Variations(t *testing.T) {
	...
	// All ON
	_, _ = db.Exec("UPDATE settings SET setting_value='true' WHERE setting_name IN ('enable_ip_blacklist', 'enable_domain_blacklist', 'block_direct_ip', 'enable_content_filtering', 'enable_waf', 'ssl_bump_enabled', 'default_password_changed', 'enable_time_restrictions')")
