...
def _propagate_blacklists(kind: str = "all"):
    """Regenerate Squid/dnsmasq files and signal reload after blacklist changes.
    kind: 'ip' (Squid only), 'domain' (dnsmasq only), 'all' (both)."""
    try:
        # Regenerate flat-file ACLs used by Squid
        export_blacklists_to_files()

        if kind in ("ip", "all"):
            # Signal Squid to reload ACLs
            try:
                requests.post(f"http://{PROXY_HOST}:5000/api/reload", timeout=5)
            except requests.RequestException:
                logger.warning("Could not signal Squid reload")

        if kind in ("domain", "all"):
            # Regenerate dnsmasq blocklist
            try:
                conn = get_db()
                cursor = conn.cursor()
                ...
                lines = ["# Auto-generated by Secure Proxy Manager", ""]
                for d in domains:
                    d = d.strip()
                ...
                logger.warning(f"Could not reload dnsmasq: {e}")
    except Exception as e:
        logger.warning(f"Blacklist propagation error: {e}")
...
def add_ip_to_blacklist(item: IPBlacklistItem, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        background_tasks.add_task(_propagate_blacklists, "ip")
...
def bulk_delete_ips(ids: list[int], background_tasks: BackgroundTasks):
    ...
    if not ids:
        return {"status": "success", "message": "No IDs provided", "data": {"deleted": 0}}
    conn = get_db()
    try:
        cursor = conn.cursor()
...
def bulk_delete_domains(ids: list[int], background_tasks: BackgroundTasks):
    ...
    try:
        ...
        conn.commit()
        if deleted:
            background_tasks.add_task(_propagate_blacklists, "domain")
        return {"status": "success", "message": f"Deleted {deleted} entries", "data": {"deleted": deleted}}
    finally:
        conn.close()
