...
export function ThreatIntel() {
  ...
  const { data: shadowIt } = useQuery<any[]>({ queryKey: ['shadow-it'], queryFn: () => api.get('analytics/shadow-it').then(r => r.data.data), refetchInterval: vis ? 60_000 : false });
  const { data: fileExts } = useQuery<any>({ queryKey: ['file-exts'], queryFn: () => api.get('analytics/file-extensions').then(r => r.data.data), refetchInterval: vis ? 60_000 : false });
  const { data: topDomains } = useQuery<any[]>({ queryKey: ['top-domains'], queryFn: () => api.get('analytics/top-domains').then(r => r.data.data), refetchInterval: vis ? 60_000 : false });
  const { data: uaData } = useQuery<any>({ queryKey: ['user-agents'], queryFn: () => api.get('analytics/user-agents').then(r => r.data.data), refetchInterval: vis ? 60_000 : false });
    ...
    <div className="space-y-4 animate-in fade-in duration-500">
      ...
      <div className="grid gap-3 lg:grid-cols-3">
                ...
                <div className="h-[120px]">
                  <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={0}>
                    <PieChart><Pie data={fileExts.categories.slice(0, 8)} dataKey="count" nameKey="category" cx="50%" cy="50%" innerRadius={30} outerRadius={50} paddingAngle={2}>
                      {fileExts.categories.slice(0, 8).map((_: any, i: number) => <Cell key={i} fill={C[i % C.length]} />)}
                    </Pie><Tooltip contentStyle={{ backgroundColor: '#1e293b', border: 'none', borderRadius: '6px', fontSize: '11px' }} /></PieChart>
                  </ResponsiveContainer>
                </div>
                <div className="flex flex-wrap gap-1 mt-1">
                  {fileExts.categories.slice(0, 6).map((c: any, i: number) => (
        ...
        <Card className="bg-card/50">
          ...
          <CardContent className="p-2">
            {uaData?.service_types?.length > 0 ? (
              <div className="space-y-1">
                {uaData.service_types.slice(0, 10).map((st: any, i: number) => {
