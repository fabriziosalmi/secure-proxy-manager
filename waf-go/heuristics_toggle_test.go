...
func TestSetHeuristicEnabledRuntime(t *testing.T) {
	withEngine(t, engine.Config{Heuristics: engine.HeuristicsFromEnv()}) // PIICounter on, max 5 by default

	body := strings.Repeat("a@b.com, ", 6) // 6 emails > max → should fire
	if _, score := eng.CheckResponseHeuristics(body); score == 0 {
		t.Fatal("expected PII heuristic to fire with PIICounter on")
	}

	if ok := eng.SetHeuristicEnabled("waf_h_pii", false); !ok {
		t.Fatal("eng.SetHeuristicEnabled(waf_h_pii) returned false for a known key")
	}
	if _, score := eng.CheckResponseHeuristics(body); score != 0 {
		t.Fatalf("expected no PII score after toggling waf_h_pii off, got %d", score)
	}
	if eng.HeuristicStates()["waf_h_pii"] {
		t.Fatal("heuristicStates still reports waf_h_pii enabled after toggle off")
	}

	if eng.SetHeuristicEnabled("waf_h_nonexistent", true) {
		t.Fatal("setHeuristicEnabled accepted an unknown key")
	}
}

func TestHeuristicsToggleHandler(t *testing.T) {
	withEngine(t, engine.Config{Heuristics: engine.HeuristicsFromEnv()})
	h := &MgmtHandlers{}
	...
	if eng.HeuristicStates()["waf_h_ghosting"] {
...
func TestHeuristicsHandlerReportsState(t *testing.T) {
	withEngine(t, engine.Config{Heuristics: engine.HeuristicsFromEnv()})
	eng.SetHeuristicEnabled("waf_h_sequence", true)
...
func TestHeuristicCfgConcurrentAccess(t *testing.T) {
	...
	for i := 0; i < 50; i++ {
		wg.Add(2)
		go func(on bool) { defer wg.Done(); eng.SetHeuristicEnabled("waf_h_entropy", on) }(i%2 == 0)
		go func() {
			defer wg.Done()
			_ = eng.Heuristics()
			eng.CheckResponseHeuristics("x@y.com")
		}()
	}
	wg.Wait()
}
