...
func TestCheckRequestHeuristics(t *testing.T) {
	e := New(Config{Heuristics: HeuristicsFromEnv()})

	// Test normal request
	res, score := e.CheckRequestHeuristics("1.2.3.4", "GET", "example.com", "/index.html", "", 0, 2.0, 2.0)
	...
	_, score = e.CheckRequestHeuristics("1.2.3.4", "GET", "example.com", "/index.html", longBody, len(longBody), 8.0, 2.0)
	if score == 0 {
	...
	// (SECURE-CONC-04).
	e.csMu.Lock()
	cs := e.getClientStateLocked("1.2.3.5")
	...
	e.heurMu.Lock()
	e.heur.BeaconingMinRequests = 5
	e.heur.BeaconingWindow = 300
	e.heurMu.Unlock()

	res, score = e.CheckRequestHeuristics("1.2.3.5", "GET", "example.com", "/index.html", "", 100, 0, 0)
	...
	// Test sharding
	e.heurMu.Lock()
	e.heur.ShardingMaxDests = 1
	e.heurMu.Unlock()
	e.CheckRequestHeuristics("1.2.3.6", "GET", "a.com", "/1", "", 0, 0, 0)
	res, _ = e.CheckRequestHeuristics("1.2.3.6", "GET", "b.com", "/2", "", 0, 0, 0)
	found = false
	for _, r := range res {
		if r.ID == "H4-SHARDING" {
			found = true
	...
	// Ghosting
	res, _ = e.CheckRequestHeuristics("1.2.3.7", "POST", "example.com", "/", "SSH-2.0-OpenSSH_8.2\n", 100, 0, 0)
	found = false
	for _, r := range res {
		if r.ID == "H6-GHOST" {
			found = true
...
func TestCheckResponseHeuristics(t *testing.T) {
	cfg := HeuristicsFromEnv()
	cfg.PIICounter = true
	cfg.PIIMaxPerResponse = 2
	e := New(Config{Heuristics: cfg})

	body := "email1@test.com, email2@test.com, email3@test.com"
	res, score := e.CheckResponseHeuristics(body)
